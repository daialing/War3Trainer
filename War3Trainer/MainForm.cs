...
namespace War3Trainer
{
    public partial class MainForm : LostForm
    {
        ...
        private void SetRightGrid(RightFunction function)
        {
            ...
            if (function == RightFunction.EditTable && viewData.Columns.Count >= 3)
            {
                viewData.Columns[2].Width = -2;
            }

            this.viewData.ResumeLayout();
            this.splitMain.Panel2.ResumeLayout();
        }
        private void MainForm_Resize(object sender, EventArgs e)
        {
            if (viewData.Visible && viewData.Columns.Count >= 3)
            {
                viewData.Columns[2].Width = -2;
            }
        ...
        private void ReplaceInputTextbox()
        {
            ...
            ListViewItem item = viewData.SelectedItems[0];

            // 获取目标单元格（第 3 列：目标数值）的相对 Bounds
            Rectangle cellBounds = item.SubItems[2].Bounds;
            ...
            txtInput.Text = item.SubItems[2].Text;
